...
func (c *SAPAPICaller) HeaderItem(headerItem *requests.HeaderItem) {
	err := c.callBillOfMaterialSrvAPIRequirementHeaderItem("MaterialBOM", headerItem)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callBillOfMaterialSrvAPIRequirementHeaderItem(api string, headerItem *requests.HeaderItem) error {
	...
	headerItem.BillOfMaterial = resBody.D.BillOfMaterial
	return nil
}
